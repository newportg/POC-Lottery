...
namespace Services
{
    public class AzureTableHelper<T> where T : ITableEntity, new()
    {
        ...
        {
            static void OperationDispatcher(TableBatchOperation batchOperation, ITableEntity entity)
            {
        ...
        public async Task DeleteByKey(CloudTable table, string key, string value)
        {
            var filter = TableQuery.GenerateFilterCondition(key, QueryComparisons.Equal, value);
            var query = new TableQuery<T>().Where(filter);
            var entities = await InternalExecuteQuery(table, query);
        ...
        public async Task<IEnumerable<T>> GetByKeyAsync(string key, string value, CloudTable table)
        {
            var filter = TableQuery.GenerateFilterCondition(key, QueryComparisons.Equal, value);
            var query = new TableQuery<T>().Where(filter);
            var entities = await InternalExecuteQuery(table, query);
        ...
        public async Task<IEnumerable<T>> GetListByKeyAsync(string key, string value, CloudTable table, string comparator)
        {
            var filter = TableQuery.GenerateFilterCondition(key, comparator, value);
            var query = new TableQuery<T>().Where(filter);
            var entities = await InternalExecuteQuery(table, query);
        ...
        public Task<TableResult> InsertOrReplace(CloudTable table, ITableEntity projection)
        {
            return table.ExecuteAsync(TableOperation.InsertOrReplace(projection));
        }

        public Task<TableResult> InsertOrMerge(CloudTable table, ITableEntity projection)
        {
